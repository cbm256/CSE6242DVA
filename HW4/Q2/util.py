...
def entropy(class_y):
    ...
    total = float(len(class_y))
    zero = class_y.count(0)
    one = class_y.count(1)
    pz = float(zero)/total
    po = float(one)/total
    if zero == 0 or one == 0:
        ...
        entropy = -(pz * math.log(pz,2) + po * math.log(po,2))
    return entropy
...
def information_gain(previous_y, current_y):
    ...
    info_gain = 0.45915
    """
    total = float(len(previous_y))
    cur = 0
    # Calculate entropy*probability for each part
    for row in current_y:
        if len(row) > 0:
            pro = float(len(row))/total
            cur += entropy(row) * pro
    info_gain = entropy(previous_y) - cur
